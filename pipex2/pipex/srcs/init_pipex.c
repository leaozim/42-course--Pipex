...
void	init_pipex(int argc, char **argv, t_pipex *data)
{
	...
	data->cmd_nb = argc - 3;
	data->process_id = ft_calloc(sizeof(int), data->cmd_nb);
	// init_pipes(data);
}
...
// 	i = -1;
// 	data->pipe_fd = ft_calloc(sizeof(int *), data->cmd_nb - 1);
// 	while (data->cmd_nb - 1 > ++i)
// 	{
// 		data->pipe_fd[i] = ft_calloc(sizeof(int), 2);
// 		status = pipe(data->pipe_fd[i]);
// 		if (status == -1)
// 			printf("Erro %d\n", status);
// 	}
// }
